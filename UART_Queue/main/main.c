...
void uart_event_task(void *param)
{
  ...
  size_t datalen;
  uint8_t *receivedbuffer = malloc(RX_BUF_SIZE);
  while (true)
  {
    if (xQueueReceive(uart_queue, &uart_event, portMAX_DELAY))
    {
      ...
      case UART_DATA:
        ...
        uart_read_bytes(UART_NUM_1, receivedbuffer, uart_event.size, portMAX_DELAY);
        printf("Received %.*s\n", uart_event.size, receivedbuffer);
        break;
      case UART_BREAK:
      ...
      case UART_PATTERN_DET:
        ...
        uart_read_bytes(UART_NUM_1, receivedbuffer, datalen - PATTERN_LEN, pdMS_TO_TICKS(100));
        uint8_t pat[PATTERN_LEN + 1];
        memset(pat, 0, sizeof(pat));
        uart_read_bytes(UART_NUM_1, pat, PATTERN_LEN, pdMS_TO_TICKS(100));
        printf("Data: %.*s === Pattern %s\n", datalen - PATTERN_LEN, receivedbuffer, pat);
        break;
      default:
        break;
      }
